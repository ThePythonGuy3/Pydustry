...
public class MenuFragment extends Fragment{
    ...
    public void build(Group parent){
        ...
        String versionText = "[#ffffffba]" + ((Version.build == -1) ? "[#4e66a3aa]minimalist edition" : (Version.type.equals("official") ? Version.modifier : Version.type) + " build " + Version.build + (Version.revision == 0 ? "" : "." + Version.revision));
        ...
        parent.fill((x, y, w, h) -> {
            ...
            float fx = (int)(Core.graphics.getWidth() / 3f * 2);
            float fy = (int)(Core.graphics.getHeight() / 2f);
    ...
    private void buildDesktop(){
        ...
        container.left();
        container.add().width(0.0001f);
        container.table(background, t -> {
            t.defaults().width(70f).height(70f);

            buttons(t,
                new Buttoni("$", Icon.play,
                ...
                new Buttoni("$", Icon.terrain, () -> checkPlay(ui.maps::show)), steam ? new Buttoni("$workshop", Icon.book, platform::openWorkshop) : null,
                new Buttoni("$", Icon.bookOpen, ui.mods::show),
                new Buttoni("$", Icon.paste, ui.schematics::show),
                new Buttoni("$", Icon.settings, ui.settings::show),
                new Buttoni("$", Icon.info, ui.about::show),
                new Buttoni("$", Icon.exit, Core.app::exit)
            );
        ...
        container.table(background, t -> {
            ...
            t.getColor().a = 0f;
            t.top();
            t.defaults().width(200f).height(70f);
